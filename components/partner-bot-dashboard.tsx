...
import type { getPartnerBotAnalyticsDashboard } from "@/lib/partner-bot-analytics";
import { cn } from "@/lib/utils";

type PartnerBotDashboardData = Awaited<
  ReturnType<typeof getPartnerBotAnalyticsDashboard>
>;

type PartnerBotDashboardProps = {
  botUsername: string;
  dashboard: PartnerBotDashboardData;
};
...
function buildFilterHref(input: {
  ...
  return `/affiliate/partner-bot/${input.botUsername}?${params.toString()}`;
}

export function PartnerBotDashboard({
  botUsername,
  dashboard,
}: PartnerBotDashboardProps) {
  return (
    <div className="min-w-0 max-w-full space-y-5 overflow-x-clip md:space-y-6">
        ...
        <div className="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-2xl font-semibold tracking-tight text-white md:text-3xl">
              Dashboard @{botUsername}
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--muted)]">
              Statistik ini hanya menghitung traffic dan video play yang
              terhubung ke bot partner ini.
            </p>
          </div>
      ...
      <section className="grid min-w-0 grid-cols-2 gap-3 xl:grid-cols-5">
        ...
        <MetricCard
          label="Partner users"
          value={dashboard.stats.partnerUsers}
          hint="User dari bot ini"
          icon={Users}
        />
        <MetricCard
          label="New users"
          value={dashboard.stats.newPartnerUsers}
        ...
        <MetricCard
          ...
          value={dashboard.stats.watchedSeries}
          hint="Judul yang ditonton"
          icon={Film}
        />
      </section>
        ...
        <Card className="min-w-0 overflow-hidden rounded-[1.65rem] border-white/8 bg-[#171719] md:glass-panel md:rounded-[2rem]">
          <CardContent className="p-4 sm:p-5 md:p-6">
            <SectionHeader
              icon={Repeat2}
              title="Engagement"
              description="Session yang terhubung ke bot partner ini."
            />
            ...
            <p className="mt-4 text-xs leading-5 text-[var(--muted-foreground)]">
              ...
              Total views memakai video play agar lebih relevan untuk drama.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="min-w-0 overflow-hidden rounded-[1.65rem] border-white/8 bg-[#171719] md:glass-panel md:rounded-[2rem]">
          <CardContent className="p-4 sm:p-5 md:p-6">
            <SectionHeader
              icon={Activity}
              title="Retention"
              description="Visitor bot partner yang kembali hari ini."
            />
